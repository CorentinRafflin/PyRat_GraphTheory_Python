...
TEAM_NAME = "BeginTravellingSalesman"
...
def preprocessing(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed):
   global listCloseCheese
   listCloseCheese=FromageDansZoneRempli(mazeMap,mazeHeight,mazeWidth, piecesOfCheese)
   listCloseCheese=listCloseCheese[:8]
   #preprocessing du voyageur
   global pathCheese, bestPath, listOfCheese, graphOfCheese
   listOfCheese=listCloseCheese #we keep the original positions of cheese
   listCloseCheese.append(playerLocation) #we add the playerLocation to piecesOfCheese
   graphOfCheese = graphCheese(listCloseCheese, mazeMap, mazeHeight, mazeWidth)
   bestPath,bestLength= travellingSalesman(graphOfCheese, playerLocation, listCloseCheese)  #use of travellingSalesman to find the bestPath
   indice0= listCloseCheese.index(bestPath[0])
   indice1= listCloseCheese.index(bestPath[1])
...
def turn(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese, timeAllowed): 
   global listCloseCheese, bestPath, graphOfCheese, pathCheese, listOfCheese
   for cheese in listCloseCheese:
      while cheese in piecesOfCheese:
         if len(pathCheese)>1:
...
#######
def FromageDansZoneRempli(mazeMap,mazeHeight,mazeWidth, piecesOfCheese):
   ZoneHeight=E(mazeHeight/3)
   ZoneWidth=E(mazeWidth/3)
   compteur=[[] for i in range(9)]
   for cheese in piecesOfCheese:
      (i,j)=cheese
      if i <= ZoneWidth:
         if j <= ZoneHeight:
            compteur[0].append((i,j))
         if j > ZoneHeight and j < 2*ZoneHeight:
            compteur[1].append((i,j))
         else:
            compteur[2].append((i,j))
      if i > ZoneWidth and i <= 2*ZoneWidth:
         if j <= ZoneHeight:
            compteur[3].append((i,j))
         if j > ZoneHeight and j < 2*ZoneHeight:
            compteur[4].append((i,j))
         else:
            compteur[5].append((i,j))
      else:
         if j <= ZoneHeight:
            compteur[6].append((i,j))
         if j > ZoneHeight and j < 2*ZoneHeight:
            compteur[7].append((i,j))
         else:
            compteur[8].append((i,j))
   maxi=0
   indicezone=None
